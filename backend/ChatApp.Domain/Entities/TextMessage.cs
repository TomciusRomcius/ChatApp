...
{
    [Required] public required string TextMessageId { get; set; }

    [Required] [MaxLength(255)] public required string Content { get; set; }
    ...
    public List<ResultError> Validate()
    {
        ...
        if (string.IsNullOrWhiteSpace(TextMessageId))
            errors.Add(new ResultError(ResultErrorType.VALIDATION_ERROR, "TextMessageId cannot be empty"));
        if (string.IsNullOrWhiteSpace(Content))
            errors.Add(new ResultError(ResultErrorType.VALIDATION_ERROR, "Content cannot be empty"));
        if (string.IsNullOrWhiteSpace(SenderId))
            errors.Add(new ResultError(ResultErrorType.VALIDATION_ERROR, "SenderId cannot be empty"));
        if (string.IsNullOrEmpty(ReceiverUserId) && string.IsNullOrEmpty(ChatRoomId))
            errors.Add(new ResultError(ResultErrorType.VALIDATION_ERROR,
                "You must define a ReceiverUserId or ChatRoomId"));
        if (!string.IsNullOrEmpty(ReceiverUserId) && !string.IsNullOrEmpty(ChatRoomId))
            errors.Add(new ResultError(ResultErrorType.VALIDATION_ERROR,
